...
- POS Integration: Follow `.agents/skills/toast/ini-integration-spec.md` for exact table schemas, column names, INSERT patterns
- DB Safety: Use atomic transactions per `.agents/skills/toast/db-safety.md` - BEGIN TRANSACTION/COMMIT with ROLLBACK on failure
- Payment: Implement `.agents/skills/toast/authorize-net.md` tokenization - NEVER store raw card data
- Loyalty: Calculate points per `.agents/skills/toast/loyalty.md` - update tblCustomer.EarnedPoints and tblPointsDetail

This ensures project-specific patterns, conventions, database constraints, and safety protocols are applied during execution.
</project_context>
