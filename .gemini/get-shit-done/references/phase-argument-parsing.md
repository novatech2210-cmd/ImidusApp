...
```bash
PHASE_INFO=$(node ./.gemini/get-shit-done/bin/gsd-tools.cjs find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node ./.gemini/get-shit-done/bin/gsd-tools.cjs roadmap get-phase "${PHASE}")
...
```bash
PHASE_DIR=$(node ./.gemini/get-shit-done/bin/gsd-tools.cjs find-phase "${PHASE}" --raw)
```
