...
```bash
ROADMAP=$(node ./.gemini/get-shit-done/bin/gsd-tools.cjs roadmap analyze)
```
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  node ./.gemini/get-shit-done/bin/gsd-tools.cjs summary-extract "$summary" --fields one_liner | jq -r '.one_liner'
done
```
...
```bash
ARCHIVE=$(node ./.gemini/get-shit-done/bin/gsd-tools.cjs milestone complete "v[X.Y]" --name "[Milestone Name]")
```
...
```bash
node ./.gemini/get-shit-done/bin/gsd-tools.cjs commit "docs: update retrospective for v${VERSION}" --files .planning/RETROSPECTIVE.md
```
...
```bash
INIT=$(node ./.gemini/get-shit-done/bin/gsd-tools.cjs init execute-phase "1")
```
...
```bash
node ./.gemini/get-shit-done/bin/gsd-tools.cjs commit "chore: complete v[X.Y] milestone" --files .planning/milestones/v[X.Y]-ROADMAP.md .planning/milestones/v[X.Y]-REQUIREMENTS.md .planning/milestones/v[X.Y]-MILESTONE-AUDIT.md .planning/MILESTONES.md .planning/PROJECT.md .planning/STATE.md
```
