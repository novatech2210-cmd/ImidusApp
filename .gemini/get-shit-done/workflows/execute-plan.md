...
```bash
node ./.gemini/get-shit-done/bin/gsd-tools.cjs phases list --type summaries --raw
...
node ./.gemini/get-shit-done/bin/gsd-tools.cjs state add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
...
```bash
node ./.gemini/get-shit-done/bin/gsd-tools.cjs roadmap update-plan-progress "${PHASE}"
```
...
```bash
node ./.gemini/get-shit-done/bin/gsd-tools.cjs commit "" --files .planning/codebase/*.md --amend
```
