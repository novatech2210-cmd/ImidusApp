...
```bash
PHASE_INFO=$(node ./.gemini/get-shit-done/bin/gsd-tools.cjs roadmap get-phase "${PHASE}")
```
...
```bash
node ./.gemini/get-shit-done/bin/gsd-tools.cjs commit "docs(${padded_phase}): generate context from PRD" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
PHASE_DESC=$(node ./.gemini/get-shit-done/bin/gsd-tools.cjs roadmap get-phase "${PHASE}" | jq -r '.section')
```
...
```bash
node ./.gemini/get-shit-done/bin/gsd-tools.cjs commit-docs "docs(phase-${PHASE}): add validation strategy"
```
