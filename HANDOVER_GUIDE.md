...
**How to get the IPA locally after build:**
Once the GitHub runner finishes building, you can download the result directly to your local machine:

1.  Go to the **Actions** tab in your GitHub repository.
2.  Click on the latest **iOS Build** run.
3.  Scroll down to **Artifacts** and click to download `ImidusCustomerApp-iOS`.
...
## 3. Local Delivery (Optional)

If you wish to package artifacts locally:

```bash
./scripts/package-local.sh
```

Builds are saved to the `delivery/` folder:

- `web/customer-ordering.tar.gz`: Built Next.js Customer Portal
- `web/admin-portal.tar.gz`: Built Next.js Admin Portal
- `backend/api-linux/`: Published Linux binaries for the Backend API
- `mobile/ImidusCustomerApp.apk`: Collected from existing local deliverables

_Note: iOS IPA builds are only available via the GitHub Actions runner as they require macOS._

---

## 4. Final Delivery Channel (AWS S3)
