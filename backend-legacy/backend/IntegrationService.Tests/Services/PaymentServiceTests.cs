...
namespace IntegrationService.Tests.Services
{
    ...
    public class PaymentServiceTests
    {
        ...
        #region RefundTransactionAsync Tests

        [Fact]
        public async Task RefundTransactionAsync_WithInvalidTransactionId_ThrowsAfterRetries()
        {
            // Arrange
            var service = new PaymentService(_sandboxSettings, _mockLogger.Object);
            var invalidTransactionId = "INVALID_TXN_ID";
            ...
            // Act & Assert
            // With invalid transaction ID, refund should throw after retries exhausted
            await Assert.ThrowsAsync<Exception>(async () =>
            {
                await service.RefundTransactionAsync(invalidTransactionId, refundAmount);
            });
        ...
        [Fact]
        public async Task RefundTransactionAsync_LogsCriticalOnFinalFailure()
        {
            // Arrange
            var service = new PaymentService(_sandboxSettings, _mockLogger.Object);
            var invalidTransactionId = "INVALID_REFUND_TXN";
            var refundAmount = 50.00m;

            // Act - attempt refund which will fail
            try
            {
                await service.RefundTransactionAsync(invalidTransactionId, refundAmount);
            }
            ...
            // Assert - verify critical log was written
            _mockLogger.Verify(
                x => x.Log(
                    LogLevel.Critical,
                    It.IsAny<EventId>(),
                    It.Is<It.IsAnyType>((v, t) => v.ToString()!.Contains("refund") || v.ToString()!.Contains("Refund")),
                    It.IsAny<Exception>(),
                    It.Is<Func<It.IsAnyType, Exception?, string>>((v, t) => true)),
                Times.AtLeastOnce);
        }
        ...
        /// <summary>
        /// Integration test placeholder - requires real Authorize.net sandbox
        /// </summary>
        [Fact(Skip = "Requires valid Authorize.net sandbox credentials and settled transaction")]
        public async Task RefundTransactionAsync_WithValidSettledTransaction_ReturnsTrue()
        {
            // This test would require:
            // 1. Real Authorize.net sandbox API credentials
            // 2. A previously settled transaction (24+ hours old)
            // 3. Network access to Authorize.net sandbox API
            //
            // Implementation:
            // var settings = Options.Create(new AuthorizeNetSettings
            ...
            // });
            // var service = new PaymentService(settings, _mockLogger.Object);
            // var result = await service.RefundTransactionAsync("SETTLED_TXN_ID", 10.00m);
            // Assert.True(result);
