...
CREATE INDEX IF NOT EXISTS IX_ActivityLogs_AdminUserId ON ActivityLogs(AdminUserId);
...
CREATE TABLE IF NOT EXISTS MenuOverlays (
    Id INT IDENTITY(1,1) PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS IX_MenuOverlays_ItemID ON MenuOverlays(ItemID);
CREATE INDEX IF NOT EXISTS IX_MenuOverlays_IsEnabled ON MenuOverlays(IsEnabled);
...
CREATE INDEX IF NOT EXISTS IX_PushCampaigns_Status ON PushCampaigns(Status);
CREATE INDEX IF NOT EXISTS IX_PushCampaigns_ScheduledDateTime ON PushCampaigns(ScheduledDateTime);

-- Birthday Rewards Configuration
CREATE TABLE IF NOT EXISTS BirthdayRewardConfigs (
    Id INT IDENTITY(1,1) PRIMARY KEY,
    IsEnabled BIT NOT NULL DEFAULT 1,
...
CREATE TABLE IF NOT EXISTS CustomerBirthdayTracking (
    Id INT IDENTITY(1,1) PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS IX_CustomerBirthdayTracking_IsBirthdayToday ON CustomerBirthdayTracking(IsBirthdayToday);

-- Terminal Bridge Transactions (for M6 integration)
CREATE TABLE IF NOT EXISTS TerminalBridgeTransactions (
    Id INT IDENTITY(1,1) PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS IX_TerminalBridgeTransactions_SalesID ON TerminalBridgeTransactions(SalesID);
CREATE INDEX IF NOT EXISTS IX_TerminalBridgeTransactions_Status ON TerminalBridgeTransactions(Status);
CREATE INDEX IF NOT EXISTS IX_TerminalBridgeTransactions_CreatedAt ON TerminalBridgeTransactions(CreatedAt);

-- IP Whitelist for admin access
CREATE TABLE IF NOT EXISTS AdminIpWhitelists (
    Id INT IDENTITY(1,1) PRIMARY KEY,
    AdminUserId INT NOT NULL,
...
CREATE INDEX IF NOT EXISTS IX_AdminIpWhitelists_AdminUserId ON AdminIpWhitelists(AdminUserId);
CREATE INDEX IF NOT EXISTS IX_AdminIpWhitelists_IpAddress ON AdminIpWhitelists(IpAddress);
...
CREATE INDEX IF NOT EXISTS IX_AdminUsers_IsActive ON AdminUsers(IsActive);
