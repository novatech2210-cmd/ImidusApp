  ...
  async (data: LoginData, {rejectWithValue}) => {
    try {
      ...
      try {
        ...
        NotificationService.setupTokenRefreshListener(response.user.customerId);
      } catch (notifError) {
  ...
  async (_, {rejectWithValue}) => {
    try {
      ...
      if (token && user) {
        ...
        try {
          await NotificationService.registerTokenWithBackend(user.customerId);
          NotificationService.setupTokenRefreshListener(user.customerId);
        } catch (notifError) {
          console.warn(
