...
export function SkeletonAvatar({
  ...
  return (
    <div
      className={`${sizeClass} bg-gray-200 rounded-full animate-pulse ${className}`}
    />
  );
}
...
export function SkeletonCard({ className = "" }: { className?: string }) {
  return (
    <div
      className={`bg-white p-6 rounded-lg border border-gray-200 space-y-4 ${className}`}
    >
    ...
    <div className="space-y-2 border border-gray-200 rounded-lg overflow-hidden">
      ...
      <div className="flex bg-gray-50 border-b border-gray-200">
        {Array.from({ length: cols }).map((_, i) => (
          <div
            key={`header-${i}`}
            className="flex-1 px-6 py-3 h-12 bg-gray-100 animate-pulse"
          />
        ))}
      </div>
