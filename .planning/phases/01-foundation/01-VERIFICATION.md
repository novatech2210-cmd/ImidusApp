...
human_verification:
  - test: "Start Docker containers and verify health endpoint"
    ...
    why_human: "Requires actual database restore with INI_Restaurant.Bak file and running containers"
  - test: "Execute SELECT query against tblItem"
    ...
    why_human: "Requires actual INI_Restaurant database (source of truth) to be restored and accessible"
...
**Phase Goal:** API connects to restored INI_Restaurant database (source of truth) with correctly aligned entity models
...
| 2 | Entity models match actual INI_Restaurant schema column names and types (verified via successful queries) | ✓ VERIFIED | Column aliases implemented (ID->ItemID, CatName->CName), PosRepository.cs uses correct SQL aliases, DatabaseConnectivityTests.cs validates query patterns |
...
|-------------|-------------|-------------|--------|----------|
| INFRA-01 | 01-01-PLAN.md | Database restored from INI_Restaurant.Bak and accessible via connection string | ✓ SATISFIED | restore-pos-db.sh script ready, docker-compose.yml has volume mounts, .env.example has connection strings |
...
| INFRA-03 | 01-02-PLAN.md | Entity models aligned to actual INI_Restaurant schema (correct column names/types) | ✓ SATISFIED | Column aliases implemented (ID->ItemID, CatName->CName), documented in PosEntities.cs (lines 10-21), validated in integration tests |
| INFRA-04 | 01-01-PLAN.md | Backend-only database created for CustomerProfile, idempotency keys, and audit logs | ✓ SATISFIED | Migration script creates IntegrationService DB with IdempotencyKeys and AuditLog tables, separate from INI_Restaurant database |
...
|------|------|---------|----------|--------|
| src/backend/db/schema/pos-schema.md | N/A | Placeholder pending .bak file | ℹ️ INFO | Schema discovery deferred until INI_Restaurant.Bak is provided - this is expected per plan design |
...
**Test:**
1. Place `INI_Restaurant.Bak` in `src/backend/db/backups/`
...
**Why human:** Requires restored INI_Restaurant database (source of truth) to validate actual schema matches entity model assumptions.
...
- Requires INI_Restaurant.Bak file placement
...
**Next steps:**
1. User provides INI_Restaurant.Bak file
